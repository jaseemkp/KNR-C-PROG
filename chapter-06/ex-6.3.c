/***************************************************************************Ex.6-3. Write a cross-referencer that prints a list of all words in a document, and for each word, a list of the line numbers on which it occurs. Remove noise words like ``the,''``and,'' and so on.
***************************************************************************/
#include<stdio.h>
#include<ctype.h>
#include<string.h>
...
struct node *addnode(struct node *p, char *word, int n);
void treeprint(struct node *p);
int getword(char *word, int lim);

#define MAXWORD 100
#define NKEYS (sizeof(nswords) / sizeof(*nswords))
struct line_node{
		int num;
...
main()
{
	...
	for(;;){
		...
		if(word[0] == '\0')
			break;
		if(binsearch(word, nswords, NKEYS) == NULL)
...
struct node *addnode(struct node *p, char *word, int n)
{
	...
	if(p == NULL){
	   ...
	   p->lines->next = NULL;
		...
		p->left = addnode(p->left, word, n);
	} else {
		p->right = addnode(p->right, word, n);
	}
	return p;
}
